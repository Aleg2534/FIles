...
public class Dwelling implements Building, Serializable {
    ...
    @Override
    public boolean equals(Object dwelling)
    {
        if(!(dwelling instanceof Dwelling))
        {
            return false;
        }
        if(getNumberFloors()!=((Dwelling) dwelling).getNumberFloors())
        {
            return false;
        }
        for(int i=0;i<getNumberFloors();i++)
        {
            if(!getFloor(i).equals(((Dwelling) dwelling).getFloor(i)))
            {
                return false;
            }
        }
    ...
    @Override
    public Object clone() {
        Building newBuilding = new Dwelling(dwellingFloors.length);
        for (int i = 0; i < dwellingFloors.length; i++) {
            newBuilding.setFloor(i, (Floor) ((DwellingFloor) getFloor(i)).clone());
        }
        return newBuilding;
    }
